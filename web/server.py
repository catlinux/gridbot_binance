...
def start_server(bot, host=None, port=None):
    ...
    if host is None: host = os.getenv('WEB_HOST', '127.0.0.1') 
    if port is None: port = int(os.getenv('WEB_PORT', 8001))
...
def get_status():
    ...
    try:
        ...
        return {
            "status": "Error", "active_pairs": [], "balance_usdc": 0, "total_usdc_value": 0, "portfolio_distribution": [], "session_trades_distribution": [], "global_trades_distribution": [], "strategies": [],
            "stats": { "session": {"trades":0,"profit":0,"best_coin":"-","uptime":"-"}, "global": {"trades":0,"profit":0,"best_coin":"-","uptime":"-"} }
        }
...
def get_balance_history_api():
    try:
        ...
        def fmt(rows): return [[r[0]*1000, round(r[1], 2)] for r in rows]
...
def get_all_orders():
    try:
        ...
        enhanced_orders = []
        active_symbols = set()
        if bot_instance: active_symbols = set(bot_instance.active_pairs)
        for o in raw_orders:
            symbol = o['symbol']
...
@app.get("/api/wallet")
def get_wallet_data():
    if not bot_instance or not bot_instance.connector.exchange: return []
    try:
        ...
        for asset, total_qty in items:
            ...
            usdc_value = 0.0
            price = 0.0
            if asset == 'USDC': usdc_value = total_qty; price = 1.0
            elif asset == 'USDT': usdc_value = total_qty; price = 1.0
            else:
                symbol = f"{asset}/USDC"
...
def liquidate_asset_api(req: LiquidateRequest):
    if not bot_instance or not bot_instance.connector.exchange: raise HTTPException(status_code=503, detail="Bot no conectado")
    asset = req.asset.upper()
    if asset == 'USDC': return {"status": "error", "message": "No se puede liquidar USDC."}
    symbol = f"{asset}/USDC"
    try:
        ...
        if total_balance > 0:
            ...
            else: raise HTTPException(status_code=400, detail="Error al ejecutar la orden de venta.")
        else: return {"status": "warning", "message": "Saldo insuficiente para vender."}
    except Exception as e:
...
def adjust_balance_api(req: BalanceAdjustRequest):
    try:
        ...
        amount = req.amount
        value_usdc = 0.0
        if asset == 'USDC' or asset == 'USDT': value_usdc = amount
