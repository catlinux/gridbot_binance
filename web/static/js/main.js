...
const fmtUSDC = (num) => { 
    ...
    return parseFloat(num).toLocaleString('ca-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); 
};
const fmtInt = (num) => { 
    if (num === undefined || num === null) return '--'; 
...
async function saveConfigForm() {
    ...
    try {
        ...
        if (res.ok) { msgBox.className = 'alert alert-success'; msgBox.innerHTML = '<i class="fa-solid fa-check-circle"></i> Configuració guardada! Recarregant...'; setTimeout(() => { location.reload(); }, 1500); } else { msgBox.className = 'alert alert-danger'; msgBox.innerText = 'Error: ' + data.detail; }
    } catch (e) { alert("Error de connexió al guardar."); }
}
...
function renderDonut(domId, data, isCurrency = false) {
    ...
    const chartData = (data && data.length > 0) ? data : [{value: 0, name: 'Sense Dades'}];
    chart.setOption({ 
        ...
        legend: { orient: 'vertical', left: '5%', top: 'center', itemGap: 15, textStyle: { color: '#6b7280' } }, 
        series: [{ type: 'pie', radius: ['40%', '85%'], center: ['60%', '50%'], label: { show: false, position: 'center' }, emphasis: { label: { show: true, fontSize: 18, fontWeight: 'bold' } }, data: chartData }] 
    });
}

// --- MODIFICAT: TAULA AMB NOVES COLUMNES I COLORS ---
async function loadGlobalOrders() {
    try {
        ...
        // Actualitzem capçaleres si calia, però ho controlem des d'aquí injectant files
        // Nota: Assegura't que el <thead> al HTML tingui les columnes correctes o actualitza'l manualment un cop si cal
        // Però com que no vull fer-te canviar el HTML cada cop, farem que coincideixi amb el que hi ha
        // Columnes HTML actuals: Parell | Tipus | Preu (USDC) | Quantitat | Acció
        // Canviarem el HTML per: Parell | Tipus | Preu Ordre | Preu Entrada | Actual | PnL | Valor | Acció
        
        // REGENEREM LA CAPÇALERA DINÀMICAMENT PER NO TOCAR HTML
        ...
        if (orders.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-3">No hi ha ordres actives</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = orders.map(o => {
            ...
            const typeBadge = isBuy ? '<span class="badge bg-success">COMPRA</span>' : '<span class="badge bg-danger">VENDA</span>';
            ...
            const actionText = isBuy ? 'Cancel·lar' : 'Vendre';
            const actionTitle = isBuy ? 'Recuperar USDC' : 'Vendre crypto a mercat';

            // Càlcul de PnL Latent (només per Vendes, ja que tenim l'actiu)
            let pnlDisplay = '-';
            let entryDisplay = '-';
            let pnlClass = '';

            if (!isBuy && o.entry_price > 0 && o.current_price > 0) {
                // Si venc ara a "current_price", guanyo o perdo respecte "entry_price"?
                ...
                entryDisplay = '<small class="text-muted">Target</small>'; // Compres no tenen entrada prèvia
            }
    ...
    } catch (e) { console.error("Error carregant ordres:", e); }
}

async function closeOrder(symbol, id, side, amount) {
    const action = side === 'buy' ? 'cancel·lar aquesta ordre' : 'VENDRE a mercat';
    if (!confirm(`Estàs segur que vols ${action}?`)) return;
...
async function loadHome() {
    try {
        ...
        document.getElementById('dash-profit-session').innerText = `${fmtUSDC(data.stats.session.profit)} $`;
        ...
        document.getElementById('dash-profit-total').innerText = `${fmtUSDC(data.stats.global.profit)} $`;
...
async function loadSymbol(symbol) {
    ...
    try {
        ...
        document.getElementById(`orders-${safe}`).innerHTML = allOrders.map(o => `<tr><td><b class="${o.side=='buy'?'text-buy':'text-sell'}">${o.side.toUpperCase()}</b></td><td>${fmtCrypto(parseFloat(o.price).toFixed(5))}</td><td>${fmtCrypto(o.amount)}</td></tr>`).join('');
        document.getElementById(`trades-${safe}`).innerHTML = data.trades.map(t => `<tr><td>${new Date(t.timestamp).toLocaleTimeString()}</td><td><span class="badge ${t.side=='buy'?'bg-buy':'bg-sell'}">${t.side}</span></td><td>${fmtCrypto(parseFloat(t.price).toFixed(5))}</td><td>${fmtUSDC(t.cost)}</td></tr>`).join('');
    } catch(e) { console.error(e); }
}
...
// Inicialització al final del JS
async function init() {
    try {
        const res = await fetch('/api/status');
        if (!res.ok) return;
            ...
            data.active_pairs.forEach(sym => {
                ...
                div.innerHTML = `
                    <div class="row">
                        ...
                        <div class="col-lg-4 mb-3"><div class="card h-100"><div class="card-header">Estat del Grid</div><div class="card-body"><div class="row g-2 text-center mb-4"><div class="col-6"><div class="bg-buy p-3 rounded"><small class="d-block fw-bold mb-1">COMPRES</small><b class="fs-3" id="count-buy-${safe}">0</b></div></div><div class="col-6"><div class="bg-sell p-3 rounded"><small class="d-block fw-bold mb-1">VENDES</small><b class="fs-3" id="count-sell-${safe}">0</b></div></div></div><ul class="list-group list-group-flush"><li class="list-group-item d-flex justify-content-between"><span>Pròx. Compra</span><b class="text-buy" id="next-buy-${safe}">--</b></li><li class="list-group-item d-flex justify-content-between"><span>Pròx. Venda</span><b class="text-sell" id="next-sell-${safe}">--</b></li></ul></div></div></div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3"><div class="card h-100"><div class="card-header">Ordres Actives</div><div class="card-body p-0 table-responsive" style="max-height:300px"><table class="table table-custom table-striped mb-0"><thead class="table-light"><tr><th>Tipus</th><th>Preu</th><th>Volum</th></tr></thead><tbody id="orders-${safe}"></tbody></table></div></div></div>
                        <div class="col-md-6 mb-3"><div class="card h-100"><div class="card-header">Històric d'Operacions</div><div class="card-body p-0 table-responsive" style="max-height:300px"><table class="table table-custom table-hover mb-0"><thead class="table-light"><tr><th>Hora</th><th>Op</th><th>Preu</th><th>Total (USDC)</th></tr></thead><tbody id="trades-${safe}"></tbody></table></div></div></div>
                    </div>`;
