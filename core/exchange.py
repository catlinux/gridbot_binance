...
class BinanceConnector:
    def __init__(self):
        ...
        try:
            ...
            log.error(f"Error carregant mercats: {e}")
            exit(1)
    ...
    def _load_config(self):
        try:
            ...
            log.error(f"Error llegint config.json5: {e}")
    ...
    def check_and_reload_config(self):
        try:
            ...
            if current_mtime > self.last_config_mtime:
                log.warning("Detectat canvi a config.json5. Recarregant...")
                new_config = self._load_config()
                if new_config:
                    self.config = new_config
                    return True
        except Exception as e:
            log.error(f"Error verificant config: {e}")
        return False
    ...
    def _connect(self):
        ...
        if not api_key or not secret_key:
            log.error("Falten les claus API a l'arxiu .env")
            exit(1)
        ...
        try:
            ...
            if use_testnet:
                ...
                log.warning("Mode TESTNET activat.")
            else:
                log.info("Mode REAL activat.")

        except Exception as e:
            log.error(f"Error inicialitzant CCXT: {e}")
            exit(1)

    def validate_connection(self):
        try:
            self.exchange.fetch_time() 
            log.success("Connexió amb Binance establerta correctament.")
            return True
        except Exception as e:
            log.error(f"Error de connexió API: {e}")
            return False
    ...
    def fetch_current_price(self, symbol):
        try:
            ...
            log.error(f"Error obtenint preu per {symbol}: {e}")
            return 0.0
    ...
    def place_order(self, symbol, side, amount, price):
        ...
        try:
            ...
            return order
             ...
             log.error(f"FONS INSUFICIENTS: {e}")
             return None
        except Exception as e:
    ...
    def cancel_all_orders(self, symbol):
        try:
            ...
            log.error(f"Error cancel·lant ordres {symbol}: {e}")
            return None
    ...
    def fetch_open_orders(self, symbol):
        try:
            ...
            log.warning(f"Error API obtenint ordres ({symbol}): {e}")
            return None 
    ...
    def fetch_candles(self, symbol, timeframe='15m', limit=50):
        try:
            ...
            log.error(f"Error obtenint espelmes: {e}")
            return []
    ...
    def fetch_my_trades(self, symbol, limit=20):
        try:
            ...
            log.error(f"Error obtenint trades: {e}")
            return []